...
test("workspace supports keyboard, theme, modes, and evidence inspection", async ({ page }, testInfo) => {
  ...
  await page.screenshot({ path: `screenshots/workspace-${testInfo.project.name}.png`, fullPage: true });
  ...
  await expect(page.getByText(/no personal data collected/i)).toBeVisible();
